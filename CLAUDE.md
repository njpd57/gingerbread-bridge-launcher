...
- **Our Bridge fork ([njpd57/bridge-launcher](https://github.com/njpd57/bridge-launcher)) adds methods** that the published types lack. They are declared by module augmentation in `src/types/bridge-fork.d.ts`, and the dev mock implements them (`ForkBridgeMock` in `src/mock/injectBridgeMockInDev.ts`), so add new fork methods to both. Always call them behind `bridgeHas()` with a fallback for stock Bridge:
  ...
  - **Our notification panel** (`src/notifications/NotificationPanel.vue`, state in `useMenuStore.isNotificationPanelOpen`, closed by back/home like the search panel): tapping the Gingerbread status bar or the power widget's notifications button opens it instead of Android's shade when the fork is present. It lists notifications in "En curso" / "Notificaciones" (`notificationPanelSections()`), opens them with `requestOpenNotification` and clears them with `requestDismissNotification`. On top, `QuickToggles.vue` (`useQuickSettingsStore`): flashlight, brightness (cycle auto/low/mid/high, `utils/brightness.ts`), auto-rotate, sync and ringer mode (`cycleRingerMode()`: normal → vibrate → silent, `requestSetRingerMode`) change directly; brightness and auto-rotate need the "Modify system settings" permission, ringer mode needs "Do Not Disturb access" (`getCanAccessNotificationPolicy`, re-read on `afterResume`); Wi-Fi and Bluetooth only open Android's panel (`requestOpenSystemPanel`), since apps can't toggle them, but show whether they're on (`useQuickSettingsStore.wifiOn` / `bluetoothOn`). There's no GPS button: it could only open a panel, so ringer mode took its place.
  - **The power control widget** (`widgets/power/PowerControlWidget.vue`) shows the original 2.3 set with the fork, with ringer mode in place of GPS (Wi-Fi, Bluetooth, ringer mode, sync, brightness; `AudioIcon`, `ringerModeIndicator()`), and lock / night mode / notifications / settings with stock Bridge. With the fork, Wi-Fi and Bluetooth show whether they're on (`useQuickSettingsStore.wifiOn` / `bluetoothOn`) though they can only open Android's panels. Its icons are in `src/home/icons/`, and the indicator bars come from `utils/indicators.ts`; lock screen and night mode are `useTogglesStore.lockScreen()` / `toggleNightMode()`, reached with the fork by long-pressing sync / brightness (those buttons stop `pointerdown` so HomeGrid's long press doesn't drag the widget).
  ...
  - Battery (`getBattery`, event `batteryChanged`) and media volume (`getMusicVolume` / `requestSetMusicVolume`, event `musicVolumeChanged`) are declared and mocked but not used yet: the status bar still reads the battery from web APIs.
...
- **Bridge swaps top and left in every window inset** (getters and events): `WindowInsetsSnapshot` is declared `(top, left, right, bottom)` but `getSnapshot()` fills it positionally as `(left, top, right, bottom)`. On the Z Flip5 in portrait the camera cutout and `statusBarsIgnoringVisibility` (33 px, at the top) arrive as `left: 33, top: 0`. It's in upstream Bridge too. **Our fork fixed it** (named arguments, fork side of `c05ed2b`), and `getWindowInsetsSwapFixed()` says whether the installed Bridge has the fix; the launcher doesn't read that flag yet. Stock Bridge and older fork builds still swap, so a Bridge `top` can't be trusted on its own: keep the existing fallbacks (`env(safe-area-inset-top)`, the minimum, the manual height). `FEATURES.md` ("Fallos de Bridge") tracks both insets bugs.
